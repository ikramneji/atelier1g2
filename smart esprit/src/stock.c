...
renderer = gtk_cell_renderer_text_new ();
	    ...
	    {  
			f=fopen("produit.txt","a+");
			while(fscanf(f,"%s %s %s %s %s %s %s\n",ref,nom,categ,quant,type,date,poid)!=EOF)
			{if (strcmp(nom,ch)==0 || strcmp(ref,ch)==0 || strcmp(categ,ch)==0 || strcmp(type,ch)==0){
				gtk_list_store_append (store, &iter);
				gtk_list_store_set(store,&iter,EREF,ref,ENOM,nom,ECATEG,categ,EQUANT,quant,ETYPE,type,EDATE,date,EPOID,poid, -1);
			}}
			fclose(f);
...
void stoq_ep(GtkWidget *liste,char y[], int stq)
{
            GtkCellRenderer *renderer;
	    GtkTreeViewColumn *column;
 	    GtkTreeIter iter;
...
renderer = gtk_cell_renderer_text_new ();
	    ...
	    {  
			...
			{if (stq == 0){
				...
				if (atoi(quant)==0){ strcpy(poid,"Stock épuise");
				gtk_list_store_append (store, &iter);
				gtk_list_store_set(store,&iter,EREF,ref,ENOM,nom,ECATEG,categ,EQUANT,quant,ETYPE,type,EDATE,date,EPOID,poid, -1);
			}}else{
			if ((strcmp(y,date)>0)){ strcpy(poid,"Date épuise");
				gtk_list_store_append (store, &iter);
				gtk_list_store_set(store,&iter,EREF,ref,ENOM,nom,ECATEG,categ,EQUANT,quant,ETYPE,type,EDATE,date,EPOID,poid, -1);
			}}}
			fclose(f);
